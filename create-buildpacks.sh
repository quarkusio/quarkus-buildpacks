...
for stack_dir in $(find ${STACKS_DIR} -type d -maxdepth 1 -mindepth 1)
do
  echo ---> Building Stack $(basename ${stack_dir})
  $DIR/build-stack.sh -p ${REPO_PREFIX} ${stack_dir}
done

for builder_dir in $(find ${BUILDERS_DIR} -type d -maxdepth 1 -mindepth 1)
do
